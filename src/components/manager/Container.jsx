...
const Container = forwardRef(function Container(props, ref) {
   ...
   async function fetchData() {
      try {
         const response = await fetch(
            `https://elevsidor.kreativlink.se/aw/arskurs-3/task-tracker/api/api.php?userid=${userid}`,
         )
   ...
   async function toggleReminder(id, reminder) {
      ...
      try {
         const response = await fetch(
            `https://elevsidor.kreativlink.se/aw/arskurs-3/task-tracker/api/api.php?id=${id}&reminder=${!reminder}`,
            {
               method: 'PATCH',
   ...
   async function removeTask(id) {
      ...
      try {
         const response = await fetch(
            `https://elevsidor.kreativlink.se/aw/arskurs-3/task-tracker/api/api.php?id=${id}`,
            {
               method: 'DELETE',
               headers: { 'Content-Type': 'application/json' },
            },
